...
module "records" {
  ...
  zone_name = var.zone_name #narendra.shop
  records = [
    {
      name    = "roboshop-${var.environment}" # roboshop-dev.narendra.shop
      type    = "A"
      alias   = {
...
resource "aws_lb_listener_rule" "frontend" {
  ...
  condition {
    host_header {
      values = ["roboshop-${var.environment}.${var.zone_name}"] #roboshop-dev.narendra.shop
    }
  }
}
